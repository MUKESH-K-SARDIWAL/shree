...
const FindContact = ({ navigation }) => {
  ...
  const [manualContact, setManualContact] = useState(null);
        ...
        Contacts.getAll()
          .then(contacts => {
            ...
            setFilterContactList(
              [...newContect].sort((a, b) => (a.value > b.value ? 1 : -1)),
            );
            setIsLoading(false);
          })
          .catch(e => {
  ...
  const checkUserHistoryType = async (item) => {
    setIsLoading(true);
    await getData(
      apiUrl.transaction_dashboard + '?' + `user_name=${item?.value}&transaction_mode=gold&transaction_type=all&filter_type=ASC_date&report_duration=all`)
      .then(resp => {
        ...
        if (resp?.data?.data.length == 0) {
          ...
          resp?.data?.data.find((val, ind) => {
            //  console.log(val.transaction_user_name,item.value)
            ...
            <View>
              <View style={[
                ...
                <TextInput
                  ...
                  // value={manualContact == '' ? '' : manualContact}
              ...
              ]}>
              {
                <AlphabetList
                  ...
                  renderCustomItem={item => (
                    item.phone != 'undefined' && <TouchableOpacity
                      onPress={() => {
